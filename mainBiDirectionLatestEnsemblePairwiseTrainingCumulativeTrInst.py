...
def usage():
    ...
    -o: One class SVM for the assgined classfier (especially -C 23 and -C 43)
    """

def run():
    try:
        opts, args = getopt.getopt(sys.argv[1:], "t:T:N:k:c:C:d:n:r:B:D:E:F:G:H:I:J:K:L:M:A:m:V:P:g:q:x:b:l:u:s:X:i:p:w:W:Q:y:f:e:o:h")
    ...
    if config.DATA_SOURCE == 0:
        ...
        if config.DATA_SOURCE == 64:
            ...
            config.PCAP_ROOT = os.path.join(config.BASE_DIR   ,'honeypatckBenattackSysdigFiles')
    ...
    for i in range(config.NUM_TRIALS):
        ...
        if config.DATA_SOURCE == 6 or config.DATA_SOURCE == 61 or config.DATA_SOURCE == 62 or config.DATA_SOURCE == 63 or config.DATA_SOURCE == 64 or config.DATA_SOURCE == 65: # honeypatch
            seedBenign = random.randint( startIndexBenign, endIndexBenign ) # test
            seedAttack = random.randint( startIndexAttack, endIndexAttack ) # test
            seedBenignTrain = startIndexBenign
            seedAttackTrain = startIndexAttack
        ...
        for clsfr in config.CLASSIFIER_LIST:
            ...
            for webpageId in webpageIds:
                ...
                if config.DATA_SOURCE == 0 or config.DATA_SOURCE == 3 or config.DATA_SOURCE == 4:
                    ...
                    if config.NUM_NON_MONITORED_SITES == -1: # closed world setting
                        ...
                        if webpageId in benignWebpageIds: # benign. open world: benign is the nonMonitored
                            ...
                            webpageTrain = Datastore.getWebpagesHoneyPatchSomePackets( [webpageId], 0, seedBenignTrain )
                            webpageTest  = Datastore.getWebpagesHoneyPatchSomePackets( [webpageId], len(config.DATA_SET_HONEYPATCH_MC_BENATTACK_BENIGN) - config.NUM_TESTING_TRACES, len(config.DATA_SET_HONEYPATCH_MC_BENATTACK_BENIGN) )
                        else: # attack. open world: attack is the monitored
                            # -t and -T will be for the attack only in case of open world
                            #webpageTrain = Datastore.getWebpagesHoneyPatch( [webpageId], seedAttack-config.NUM_TRAINING_TRACES, seedAttack )
                            #webpageTest  = Datastore.getWebpagesHoneyPatch( [webpageId], seedAttack, seedAttack+config.NUM_TESTING_TRACES )
                            webpageTrain = Datastore.getWebpagesHoneyPatch( [webpageId], 0, seedAttackTrain )
                            webpageTest  = Datastore.getWebpagesHoneyPatch( [webpageId], len(config.DATA_SET_HONEYPATCH_MC_BENATTACK_ATTACK) - config.NUM_TESTING_TRACES, len(config.DATA_SET_HONEYPATCH_MC_BENATTACK_ATTACK) )
                elif config.DATA_SOURCE == 65:
                    # -u option (config.NUM_NON_MONITORED_SITES) is used for the open world for honeypatch data as the number of instances
                    if config.NUM_NON_MONITORED_SITES == -1: # closed world setting
                        if webpageId in benignWebpageIds: # benign
                        ...
                        if webpageId in benignWebpageIds: # benign. open world: benign is the nonMonitored
                            ...
                            webpageTrain = Datastore.getWebpagesHoneyPatchSysdig( [webpageId], 0, seedBenignTrain )
                            webpageTest  = Datastore.getWebpagesHoneyPatchSysdig( [webpageId], len(config.DATA_SET_HONEYPATCH_MC_BENATTACK_BENIGN) - config.NUM_TESTING_TRACES, len(config.DATA_SET_HONEYPATCH_MC_BENATTACK_BENIGN) )
                        else: # attack. open world: attack is the monitored
                            # -t and -T will be for the attack only in case of open world
                            #webpageTrain = Datastore.getWebpagesHoneyPatchSysdig( [webpageId], seedAttack-config.NUM_TRAINING_TRACES, seedAttack )
                            #webpageTest  = Datastore.getWebpagesHoneyPatchSysdig( [webpageId], seedAttack, seedAttack+config.NUM_TESTING_TRACES )
                            webpageTrain = Datastore.getWebpagesHoneyPatchSysdig( [webpageId], 0, seedAttackTrain )
                            webpageTest  = Datastore.getWebpagesHoneyPatchSysdig( [webpageId], len(config.DATA_SET_HONEYPATCH_MC_BENATTACK_ATTACK) - config.NUM_TESTING_TRACES, len(config.DATA_SET_HONEYPATCH_MC_BENATTACK_ATTACK) )
                ...
                if ((config.DATA_SOURCE == 63 or config.DATA_SOURCE == 64 or config.DATA_SOURCE == 65) and webpageId in attackWebpageIds):
                    offset = config.HP_OFFSET_BFRHP_AFTRHP
                    if webpageId < config.HP_OFFSET_HP_ATTACK_STARTING_ID:
                        if config.NUM_HP_DCOY_ATTACKS_TRAIN == -1 and config.NUM_HP_DCOY_ATTACKS_TEST == -1:
                            ...
                            if webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TRAIN) \
                                and webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TEST):
                                #webpageList = [webpageTrain, webpageTest]
                                pass
                            elif not webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TRAIN) \
                                and webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TEST):
                                webpageList = [webpageTest]
                                i = 1 # so the trace will go to the testing arff file only
                            elif webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TRAIN) \
                                and not webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TEST):
                                webpageList = [webpageTrain]
                                i = 0 # so the trace will go to the training arff file only
                            else:
                        ...
                        if config.NUM_HP_DCOY_ATTACKS_TRAIN == -1 and config.NUM_HP_DCOY_ATTACKS_TEST == -1:
                            ...
                            # Few of the HP_DCOY attacks
                            if webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TRAIN) + offset: #(k - Q + w) + 12:
                                #webpageList = [webpageTrain, webpageTest]
                                pass
                            else:
                                webpageList = [webpageTest]
                            ...
                            # Few of the HP_DCOY attacks
                            if webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TEST) + offset:
                                #webpageList = [webpageTrain, webpageTest]
                                pass
                            else:
                                webpageList = [webpageTrain]
                            ...
                            # Few of the HP_DCOY attacks
                            if webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TRAIN) + offset \
                                and webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TEST) + offset:
                                #webpageList = [webpageTrain, webpageTest]
                                pass
                            elif not webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TRAIN) + offset \
                                and webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TEST) + offset:
                                webpageList = [webpageTest]
                                i = 1 # so the trace will go to the testing arff file only
                            elif webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TRAIN) + offset \
                                and not webpageId < (config.BUCKET_SIZE - config.NUM_HP_DCOY_ATTACKS_TOTAL + config.NUM_HP_DCOY_ATTACKS_TEST) + offset:
                                webpageList = [webpageTrain]
                                i = 0 # so the trace will go to the training arff file only
                            else:
                                continue # webpageId won't be added to either training or testing set. Grab next webpageId
