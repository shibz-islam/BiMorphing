...
# same as mainBiDirectionLatestEnsembleNewData.py but reshuffling IDS traces
            ...
            for webpageId in webpageIds:
                ...
                for w in webpageList: # was for w in [webpageTrain, webpageTest]:
                    for trace in w.getTraces():
                        #if countermeasure:
                        #if countermeasure and i == 1: # morph testing packets only

                        if i == 0: # training instances
                            instanceCtr += 1
                            if countermeasure and instanceCtr % 2 == 0: # morph some packets
                                if config.COUNTERMEASURE in [config.DIRECT_TARGET_SAMPLING, config.WRIGHT_STYLE_MORPHING]:
