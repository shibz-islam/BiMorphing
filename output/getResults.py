		    ...
		    for fileLine in fileLines:
		        if not fileLine.startswith("tpr"):
		            lineResults = fileLine.split(",") # [tpr	, fpr	, Acc	, F2	, tp	, tn	, fp	, fn	, File ID]
                            mylineresult[0].append(float(lineResults[0]))#new
			    mylineresult[1].append(float(lineResults[1]))#new
                            mylineresult[2].append(float(lineResults[2]))#new
			    mylineresult[3].append(float(lineResults[3]))#new
                            lineResults[0] = min(mylineresult[0])#new
                            lineResults[1] = min(mylineresult[1])#new
			    lineResults[2] = min(mylineresult[2])#new
			    lineResults[3] = min(mylineresult[3])#new
