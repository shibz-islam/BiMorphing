...
class Datastore:
    ...
    @staticmethod
    def writeWangTorFile( webpageId, traceIndex, trace ):
        if config.DATA_SOURCE == 5:
            folder_path = os.path.join(config.WANG, "wang-"+str(config.COUNTERMEASURE))
            if not os.path.exists(folder_path):
                os.mkdir(folder_path)
            if webpageId < 100:  # Monitored
                file_path = os.path.join(folder_path, str(webpageId)+"-"+str(traceIndex))
            else: #nonMonotored
                file_path = os.path.join(folder_path, str(webpageId))
            ...
            for packet in trace.getPackets():
                if packet.getDirection() == Packet.DOWN:
                    ...
                    direction = 1
                term = str(packet.getTime()) + "\t" + str(direction * packet.getLength())
                fout.write(term + "\n")
            fout.close()
