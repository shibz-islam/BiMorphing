...
from clustering.BiDiClustering import BiDiClustering
from clustering.HpNGramClustering import HpNGramClustering
...
def intToClassifier(n):
    ...
    if n == config.LIBERATORE_CLASSIFIER:
        ...
        classifier = HpNGram
    elif n == config.BI_DI_CLUSTERING:
        classifier = BiDiClustering
    elif n == config.HP_NGRAM_CLUSTERING:
        classifier = HpNGramClustering
...
def usage():
    ...
    -C [int] : classifier to run, if multiple classifiers, then separate by a comma (example: -C 23,3,15)
        ...
        123:Bi-Di with clustering
        143:HoneyPatch Sysdig Classifier (NGram) with clustering
        (default 0)
    ...
    -R: Kernel type for one-class svm (0: linear, 1:polynomial, 2:Radial Basis Function, 3: sigmoid) (default: 2)
    -U: Nu parameter for one-class svm (default: 0.5)

    -S: [int] Clustering method for outlier detection.
        1: kmeans++
        2: pca + kmeans++
        3: pca + kmeans++ + convex hull
        4: pca + kmeans++ + convex hull + mixed
        5: pca + kmeans++ + convex hull + knn
        (default: 1)
    """

def run():
    try:
        opts, args = getopt.getopt(sys.argv[1:], "t:T:N:k:c:C:d:n:r:B:D:E:F:G:H:I:J:K:L:M:A:m:V:P:g:q:x:b:l:u:s:X:i:p:w:W:Q:y:f:e:o:R:U:S:h")
    ...
    for o, a in opts:
        if o in ("-k"):
            ...
            config.SVM_KERNEL = int(a)
        elif o in ("-U"):
            config.OC_SVM_Nu = float(a)
        elif o in ("-S"):
            config.CLUSTERING_METHOD = int(a)
        else:
            usage()
            sys.exit(2)
    ...
    config.OUTPUT_DIR = config.OUTPUT_DIR+'.'+config.CLUSTERING[config.CLUSTERING_METHOD]
    ...
    for i in range(config.NUM_TRIALS):
        ...
        if config.DATA_SOURCE == 6 or config.DATA_SOURCE == 61 or config.DATA_SOURCE == 62 or config.DATA_SOURCE == 63 or config.DATA_SOURCE == 64 or config.DATA_SOURCE == 65: # honeypatch
            ...
            seedBenignTrain = startIndexBenign # for Exp3
            seedAttackTrain = startIndexAttack # for Exp3
...
###
        ...
        for clsfr in config.CLASSIFIER_LIST:
            ...
            if config.NUM_NON_MONITORED_SITES != -1 and \
                    (config.DATA_SOURCE == 62 or config.DATA_SOURCE == 63 or config.DATA_SOURCE == 64 or config.DATA_SOURCE == 65) and \
                    (config.CLASSIFIER != config.BI_DI_CLUSTERING and config.CLASSIFIER != config.HP_NGRAM_CLUSTERING): # honeypatch dataset, calculate TPR, FPR, F2, ...etc. Results calculated in other places for clustering
                positive = [] # attacks are positive
