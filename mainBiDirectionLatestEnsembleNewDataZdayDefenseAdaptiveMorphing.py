...
def usage():
    ...
    -v: How much of the training to be morphed, see mainBiDirectionLatestEnsembleNewDataZdayDefenseAdaptiveMorphing.py
...
def run():
    try:
        opts, args = getopt.getopt(sys.argv[1:], "t:T:N:k:c:C:d:n:r:B:D:E:F:G:H:I:J:K:L:M:A:m:V:P:g:q:x:b:l:u:s:X:i:p:w:W:Q:y:f:e:a:o:R:v:h")
    ...
    for o, a in opts:
        if o in ("-k"):
            ...
            config.TRAIN_MORPH_CONSTANT = int(a)
        else:
            usage()
            sys.exit(2)
    ...
    for i in range(config.NUM_TRIALS):
        ...
        if config.DATA_SOURCE == 62 or config.DATA_SOURCE == 63 or config.DATA_SOURCE == 64 or config.DATA_SOURCE == 65:
            ...
            if (config.ATTACK_LIST): # list not empty
                ...
                random.shuffle(benignWebpageIds) # added for defenses so target distr. changes
                ...
                for w in webpageList: # was for w in [webpageTrain, webpageTest]:
                    for trace in w.getTraces():
                        ...
                        if i == 0:  # training instances
                            instanceCtr += 1
                            # if config.TRAIN_MORPH_CONSTANT = 1, then morph all traning traces,
                            # 2, morph half of the traning traces, 4, morph quarter of the training traces
                            if countermeasure and instanceCtr % config.TRAIN_MORPH_CONSTANT == 0:  # morph some packets
