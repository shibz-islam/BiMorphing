...
def generateModel(traces):
            ...
            for trace in traces:
                for packet in trace.getPackets():
                    ...
                    linesentence = linesentence + " " + str(packet.getLength()) + "_" + str(packet.getDirection())
                    # reset sentences
                    if mypackCount % 40 == 0:
    ...
    for i in range(config.NUM_TRIALS):
        ...
        for webpageId in webpageIds:
            ...
            for w in [webpageTrain, webpageTest]:
                ...
                i += 1
            traintracesofWebsite1 = traintracesofWebsite[:len(traintracesofWebsite)/2]
            traintracesofWebsite2 = traintracesofWebsite[len(traintracesofWebsite)/2:]
            testtracesofWebsite1 = testtracesofWebsite[:len(testtracesofWebsite)/2]
            testtracesofWebsite2 = testtracesofWebsite[len(testtracesofWebsite)/2:]

            generateModel(traintracesofWebsite)
            for trace in traintracesofWebsite1:
                instance = classifier.traceToInstance(trace)
                if instance:
                    trainingSet.append(instance)
            #generateModel(testtracesofWebsite1)
            for trace in testtracesofWebsite1:
                instance = classifier.traceToInstance(trace)
                if instance:
                    testingSet.append(instance)
        

            #generateModel(traintracesofWebsite2)
            for trace in traintracesofWebsite2:
                trace.setId(trace.getId()+1)
                instance = classifier.traceToInstance(trace)
                if instance:
                    trainingSet.append(instance)
            #generateModel(testtracesofWebsite2)
            for trace in testtracesofWebsite2:
                trace.setId(trace.getId()+1)
                instance = classifier.traceToInstance(trace)
                if instance:
                    testingSet.append(instance)
