...
# same as mainBiDirectionLatestEnsembleNewDataDefenses.py but reshuffling IDS traces
...
from DirectTargetSamplingIDS import DirectTargetSamplingIDS
...
def intToCountermeasure(n):
    ...
    elif n==config.DIRECT_TARGET_SAMPLING_IDS:
        countermeasure = DirectTargetSamplingIDS
    elif n > 10:
...
def usage():
    ...
    -c [int]: countermeasure to use
        ...
        9: Traffic Morphing
        81: Direct Target Sampling _ IDS
        (default 0)
...
def run():
    ...
    for i in range(config.NUM_TRIALS):
        ...
        if config.DATA_SOURCE == 62 or config.DATA_SOURCE == 63 or config.DATA_SOURCE == 64 or config.DATA_SOURCE == 65:
            ...
            # For the HP_IDS, shuffle the ids again so the target trace (for defenses) changes in every run
            random.shuffle( webpageIds )
        ...
        for clsfr in config.CLASSIFIER_LIST:
            ...
            for webpageId in webpageIds:
                ...
                if targetWebpage == None:
                    ...
                    print "Target ID: " + str(targetWebpage.getId())
                ...
                metadata = None
                if config.COUNTERMEASURE in [config.DIRECT_TARGET_SAMPLING, config.WRIGHT_STYLE_MORPHING, config.DIRECT_TARGET_SAMPLING_IDS]:
                ...
                for w in webpageList: # was for w in [webpageTrain, webpageTest]:
                    for trace in w.getTraces():
                        #if countermeasure:
                        #if countermeasure and webpageId in attackWebpageIds and i == 1: # for hp_ids, morphing attacks (testing) traces
                        instanceCtr += 1
                        if countermeasure and webpageId in attackWebpageIds:
                            if (i == 1) or (i == 0 and instanceCtr % 4 == 0): # morph all attack testing and some training insts
                                if config.COUNTERMEASURE in [config.DIRECT_TARGET_SAMPLING, config.WRIGHT_STYLE_MORPHING, config.DIRECT_TARGET_SAMPLING_IDS]:
            ...
            output.append(str(targetWebpage.getId()))
