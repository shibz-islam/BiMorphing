...
class LeNetConvPoolLayer_2_svm(object):
...
def run_convNet(datasets, n_visible, n_out, batch_size, n_hidden, learning_rate,  n_epochs,
                    nkerns=[20, 50]):
    ...
    layer0_input = x.reshape((batch_size, 1, imageDim, imageDim))
    ...
    layer0 = LeNetConvPoolLayer_2_svm(
        rng,
        input=layer0_input,
        #image_shape=(batch_size, 1, 28, 28), # *
        image_shape=(batch_size, 1, imageDim, imageDim),
        filter_shape=(nkerns[0], 1, 5, 5), # *
        poolsize=(2, 2) # *
    )
    imageDim2 = imageDim - 5 + 1
    imageDim2 = imageDim2 / 2

    # Construct the second convolutional pooling layer
    # filtering reduces the image size to (12-5+1, 12-5+1) = (8, 8)
    # maxpooling reduces this further to (8/2, 8/2) = (4, 4)
    # 4D output tensor is thus of shape (batch_size, nkerns[1], 4, 4)
    layer1 = LeNetConvPoolLayer_2_svm(
        rng,
        input=layer0.output,
        #image_shape=(batch_size, nkerns[0], 12, 12), # *
        image_shape=(batch_size, nkerns[0], imageDim2, imageDim2),
    ...
    layer3 = OVASVMLayer(input=layer2.output, n_in=batch_size, n_out=n_out)


    # the cost we minimize during training is the NLL of the model
    #cost = layer3.negative_log_likelihood(y)
    cost = layer3.ova_svm_cost(y1)
...
def runDL(files):
    ...
    train_set_x, train_set_y, train_set_y1 = Utils_2.shared_dataset_svm(train_Xy) # theano shared datasets
    valid_set_x, valid_set_y, valid_set_y1 = Utils_2.shared_dataset_svm(valid_Xy)  # theano shared datasets
    test_set_x, test_set_y, test_set_y1 = Utils_2.shared_dataset_svm(test_Xy) # theano shared datasets
    datasets = [(train_set_x, train_set_y, train_set_y1), (valid_set_x, valid_set_y, valid_set_y1),(test_set_x, test_set_y, test_set_y1)]
    ...
    return run_convNet(datasets, n_visible, n_out, batch_size, n_hidden, learning_rate,  n_epochs)
