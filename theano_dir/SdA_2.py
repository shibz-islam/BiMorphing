...
def test_SdA(finetune_lr=0.1, pretraining_epochs=15,
    ...
    sda = SdA_2(
        numpy_rng=numpy_rng,
        n_ins=28 * 28, # * number of original features ( d ). * means change here
        hidden_layers_sizes=[1000, 1000, 1000], # number of abstract features ( d') for each dA. *
        n_outs=10 # number of classes *
    )
...
def runDL(files):
    print("Running SdA with logistic regression")
    print("###################################")
    ratio_valid = 1/5.0 # ratio of training from validation set
    #[train_Xy, valid_Xy, test_Xy, n_dimensions, classes, numClasses] = \
    #    Utils_2.getDataIntLabelsClipValues(files, ratio_valid) # [train_Xy, test_Xy]; train_Xy and test_Xy are tuples of (X: ndarray(float32), y: ndarray(int64).

    [train_Xy, valid_Xy, test_Xy, n_dimensions, classes, numClasses] = \
        Utils_2.getDataIntLabels(files, ratio_valid)
    ...
    corruption_level = float(config.DEEP_LEARNING_PARAMETERS['corruption_level']) #0.3

    #datasets, n_visible, n_hidden, corruption_level,

    #hidden_layers_sizes = [n_hidden, n_hidden, n_hidden] #[1000, 1000, 1000]
    ...
    #corruption_levels = [.01, .2, .3] # def: [.1, .2, .3]
    ...
    #pretrain_lr = 0.001 #0.01 #0.001
    pretrain_lr = float(config.DEEP_LEARNING_PARAMETERS['pretrain_lr'])
    #pretraining_epochs = 200 #100 #500 #100 #20 # for dAEs
    ...
    #training_epochs = 200 #100 #500 #100 #20 #1000
