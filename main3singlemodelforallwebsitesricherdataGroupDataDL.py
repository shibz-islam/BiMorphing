...
from GloveClassifier2 import GloveClassifier2
...
def intToClassifier(n):
    ...
    elif n == config.GLOVE_CLASSIFIER2:
        classifier = GloveClassifier2
    return classifier
...
def usage():
    ...
    -C [int] : classifier to run
        ...
        16: GloVe Classifier
        116: GloVe Classifier 2. Concat all words vectors for Deep Learning
        (default 0)
...
def run():
    try:
        opts, args = getopt.getopt(sys.argv[1:], "t:T:N:k:c:C:d:n:r:B:D:E:F:G:H:I:J:K:L:M:A:V:b:m:z:Z:h")
    ...
    if not os.path.exists(outputFilename + '.output'):
        banner = ['accuracy', 'overhead', 'timeElapsedTotal', 'timeElapsedClassifier', 'RunID', 'DL_Method']
    ...
    for i in range(config.NUM_TRIALS):
        ...
        # START. Block shifted one tab to the right to include multiplooe Deep Learning methods from the same instances. Apr 11, 2017
        for dlMethod in config.DEEP_LEARNING_METHOD_LIST:
            ...
            # END. Block shifted one tab to the right to include multiple Deep Learning methods from the same instances. Apr 11, 2017
