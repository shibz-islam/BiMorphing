...
def usage():
    ...
    -c [int]: countermeasure to use, if multiple countermeasures, then separate by a comma (example: -c 0,8,9)
        0: None
        1: Pad to MTU
        ...
        while dataAvailable:
            for span in config.COVARIATE_SHIFT_LIST:
                ...
                for clsfr in config.CLASSIFIER_LIST:
                    for cntrmsre in config.COUNTERMEASURE_LIST: # Mar 17, 2017
                        #if breakClassifier:
                        #    breakClassifier = False
                        #    break
                        preCountermeasureOverhead = 0
                        ...
                        config.COUNTERMEASURE = int(cntrmsre) # Mar 17, 2017
                        countermeasure = intToCountermeasure(config.COUNTERMEASURE)
                ...
                # START - Blocks shifted left twice for multiple countermeasures (Mar 17, 2017)
                spanCtr += 1
                ...
                if accuracy <= config.ACC_THRESHOLD:
                    ...
                    if config.START_SLIDE_TRAIN+config.NUM_TRAINING_TRACES+config.COVARIATE_SHIFT+config.NUM_TESTING_TRACES >= len(config.DATA_SET):
                        dataAvailable = False
                # END - Blocks shifted left twice for multiple countermeasures (Mar 17, 2017)
