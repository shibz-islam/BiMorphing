...
# Difference between ...DL.py and ...DL2.py is running multiple DL methods with the same classes and instances
import sys
import config
import time
import os
...
from TfidfClassifierHpNGram import TfidfClassifierHpNGram
...
def intToClassifier(n):
    ...
    elif n == config.HP_NGRAM_TFIDF:
        classifier = TfidfClassifierHpNGram
    '''
...
def usage():
    ...
    -C [int] : classifier to run, if multiple classifiers, then separate by a comma (example: -C 23,3,15)
        ...
        23: Bi-Di/BIND. Adversarial Classifier Using BiDirection features only (no panch features)
        ...
        43: HoneyPatch Sysdig Classifier (HpNGram)
        223: Bi-Di/BIND with TFIDF
        443: HpNGram with TFIDF
        (default 0)
...
def run():
    try:
        opts, args = getopt.getopt(sys.argv[1:], "t:T:N:k:c:C:d:n:r:B:D:E:F:G:H:I:J:K:L:M:A:m:V:P:g:q:x:b:l:u:s:X:i:p:w:W:Q:y:f:e:a:o:R:z:Z:h")
    ...
    for i in range(config.NUM_TRIALS):
        ...
        #print 'WARNING: You are using the same set of websites!'
        #webpageIds = [553,698,441,640,130,141,431,251,738,357,563,398,773,461,427,134,691,66,311,228]
        #print webpageIds
        ...
        for clsfr in config.CLASSIFIER_LIST:
            ...
            if config.ENSEMBLE != 0:
                ...
                if config.CLASSIFIER == 23:
                    ...
                    #config.PCAP_ROOT = os.path.join(config.BASE_DIR,'honeypatckBenattackSysdig')
                    #config.PCAP_ROOT = os.path.join(config.BASE_DIR,'honeypatckBenattackSysdigMerge')
                elif config.CLASSIFIER == 43 or config.CLASSIFIER == 443:
                    config.DATA_SOURCE=65
                    if (HP_DATASET_KEY == -1):
                        #config.PCAP_ROOT = os.path.join(config.BASE_DIR,'honeypatckBenattackSysdig')
                        #config.PCAP_ROOT = os.path.join(config.BASE_DIR,'honeypatckBenattackSysdigTest')
            ...
            if not os.path.exists(outputFilename+'.output'):
                banner = ['accuracy','overhead','timeElapsedTotal','timeElapsedClassifier','fileId', 'DLMethod']
            ...
            for webpageId in webpageIds:
                ...
                if config.DATA_SOURCE == 0 or config.DATA_SOURCE == 3 or config.DATA_SOURCE == 4:
                    ...
                    if not unMonitoredWebpageIdsObj.__contains__(webpageId):
                        print 'If closed-world, make sure -N <=100. Otherwise, instances of websites with IDs > ' \
                              '100 will be repeated. ' \
                              'In this experiment, -N is ' + str(config.TOP_N) + '.'
