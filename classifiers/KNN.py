...
class kNN():
    ...
    def _majority_vote(self, points):
        ...
        if config.NUM_MONITORED_SITES == -1 and config.NUM_NON_MONITORED_SITES == -1:
            ...
            # if len(votes.keys()) == 1 and votes[config.binaryLabels[0]] > 0:
            #     className = config.binaryLabels[0] #webpageMon
            # elif len(votes.keys()) == 1 and votes[config.binaryLabels[1]] > 0:
            #     className = config.binaryLabels[0]  # webpageMon
            # else:
    ...
    def classify(runID, trainingSet, testingSet):
        ...
        if config.CROSS_VALIDATION == 0:
            ...
            trainInstancesList, testInstancesList, yTrain, yTest = train_test_split(fileInstancesList, y, test_size=6000,
                                                                                    random_state=42)
