...
def get_pkt_list(trace_data):
    data = []
    for packet in trace_data.getPackets():
        packetDirection = packet.getDirection()
        packetTimestamp = packet.getTime()

        if packetDirection == Packet.UP:
            data.append((packetTimestamp, 1))
        else:
            data.append((packetTimestamp, -1))
    return data
...
def insertArrayIntoInstance(instance, key, array, limit):
    count = 0
    for value in array:
        count += 1
        k = key + '-' + str(count)
...
def TOTAL_FEATURES(trace_data, max_size=175):
    ...
    instance = insertArrayIntoInstance(instance, 'intertimestats', intertimestats, 0)
    instance = insertArrayIntoInstance(instance, 'timestats', timestats, 0)
    instance = insertArrayIntoInstance(instance, 'number_pkts', number_pkts, 0)
    instance = insertArrayIntoInstance(instance, 'thirtypkts', thirtypkts, 0)
    instance['stdconc'] = stdconc
    instance['avgconc'] = avgconc
    instance['avg_per_sec'] = avg_per_sec
    instance['std_per_sec'] = std_per_sec
    instance['avg_order_in'] = avg_order_in
    instance['avg_order_out'] = avg_order_out
    instance['std_order_in'] = std_order_in
    instance['std_order_out'] = std_order_out
    instance['medconc'] = medconc
    instance['med_per_sec'] = med_per_sec
    instance['min_per_sec'] = min_per_sec
    instance['max_per_sec'] = max_per_sec
    instance['maxconc'] = maxconc
    instance['perc_in'] = perc_in
    instance['perc_out'] = perc_out
    instance = insertArrayIntoInstance(instance, 'altconc', altconc, 0)
    instance = insertArrayIntoInstance(instance, 'alt_per_sec', alt_per_sec, 0)
    instance['sum-altconc'] = sum(altconc)
    instance['sum-alt_per_sec'] = sum(alt_per_sec)
    instance['sum-intertimestats'] = sum(intertimestats)
    instance['sum-timestats'] = sum(timestats)
    instance['sum-number_pkts'] = sum(number_pkts)


    diff= max_size - len(instance.keys())
    if diff>0:
        instance = insertArrayIntoInstance(instance, 'conc', conc, diff)

    diff = max_size - len(instance.keys())
    if diff>0:
        instance = insertArrayIntoInstance(instance, 'per_sec', per_sec, diff)

    # print "Total features: " + str(len(instance.keys()))
