...
def run():
    try:
        opts, args = getopt.getopt(sys.argv[1:], "t:T:N:k:c:C:d:n:r:B:D:E:F:G:H:I:J:K:L:M:A:m:V:P:g:q:x:b:l:u:s:X:i:p:h")
    ...
    for i in range(config.NUM_TRIALS):
        ...
        if config.DATA_SOURCE == 6 or config.DATA_SOURCE == 61 or config.DATA_SOURCE == 62 or config.DATA_SOURCE == 63 or config.DATA_SOURCE == 64: # honeypatch
        ...
        benignWebpageIds = []
        attackWebpageIds = []
        if config.DATA_SOURCE == 62 or config.DATA_SOURCE == 63 or config.DATA_SOURCE == 64:
...
###
        ...
        for clsfr in config.CLASSIFIER_LIST:
            ...
            if config.NUM_NON_MONITORED_SITES != -1 and (config.DATA_SOURCE == 5 or config.DATA_SOURCE == 41 or config.DATA_SOURCE == 42 \
                                                      ...
                                                      or config.DATA_SOURCE == 63 or config.DATA_SOURCE == 64):
            ...
            if config.NUM_NON_MONITORED_SITES != -1 and (config.DATA_SOURCE == 62 or config.DATA_SOURCE == 63 or config.DATA_SOURCE == 64): # honeypatch dataset, calculate TPR, FPR, F2, ...etc
                positive = [] # attacks are positive
